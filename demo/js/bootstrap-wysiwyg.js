...
(function ($) {
	...
	$('#submit').on('click', function(e){
       ...
       e.preventDefault()
        ...
        let answerContent = $.trim($('#editor').text())
        let time = ''
			...
			insertFiles = function (files) {
				...
				/*$.each(files, function (idx, fileInfo) {
					if (/^image\//.test(fileInfo.type)) {
							...
							var name = 'AnwserDetail.jpg';				

							var file = new AV.File(name, localFile)
							newAnswer.set('imageFile', file)
							
							file.save().then(function(file) {
								console.log('save')
								// 文件保存成功								
								console.log(newAnswer.get('imageFile'))
								//获取缩略图地址
								var newImage = $('<img>').attr('src', file.url())
								newImage.width('200px')
								console.log(newImage)
								$('.answerContent').append(newImage)
								newAnswer.set('imageFile', file)
								newAnswer.save().then(function(answer){
									...
									if(answerId){
										...
										query.get(answerId).then(function (todo) {
											/*将数据存入数据库*/
									/*	newAnswer.set('authorName', answerAuthor)
										newAnswer.set('content', answerContent)
										newAnswer.set('time', time)
